...
class BatchBeam(NamedTuple):
    ...
    def ids(self):
        if self.state is None:
            raise AttributeError("BatchBeam.state is None, cannot access ids.")
        return self.state.ids.view(-1)  # Need to flat as state has steps dimension

    def __getitem__(self, key):
        if self.state is None:
            raise AttributeError("BatchBeam.state is None, cannot access __getitem__.")
    ...
    def initialize(state):
        ...
        return BatchBeam(
            ...
            state=state,
            parent=torch.tensor([], dtype=torch.int64, device=device),
            action=torch.tensor([], dtype=torch.int64, device=device),
    ...
    def propose_expansions(self):
        if self.state is None:
            raise AttributeError(
                "BatchBeam.state is None, cannot access propose_expansions."
            )
    ...
    def expand(self, parent, action, score=None):
        if self.state is None:
            raise AttributeError("BatchBeam.state is None, cannot access expand.")
        return self._replace(
    ...
    def all_finished(self):
        if self.state is None:
            raise AttributeError("BatchBeam.state is None, cannot access all_finished.")
        return self.state.all_finished()

    def cpu(self):
        if self.state is None:
            raise AttributeError("BatchBeam.state is None, cannot access cpu.")
        return self.to(torch.device("cpu"))

    def to(self, device):
        if self.state is None:
            raise AttributeError("BatchBeam.state is None, cannot access to.")
    ...
    def clear_state(self):
        if self.state is None:
            raise AttributeError("BatchBeam.state is None, cannot access clear_state.")
        return self._replace(state=None)

    def size(self):
        if self.state is None:
            raise AttributeError("BatchBeam.state is None, cannot access size.")
...
def segment_topk_idx(x, k, ids):
    ...
    group_offsets = splits.new_zeros((int(splits.max().item()) + 1,))
